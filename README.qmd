---
title: "Paramecium Pavlovian Conditioning <br> Intro to the data"
author: "Daniel Fürth & Waked Hamed"
format: 
  gfm:
    toc: true
fig-dpi: 300
...
✅ indicate data that have been processed and deposited into the `./data/` folder.
...
I aim to minimize package dependencies in my R projects, relying solely on base R. Nevertheless, I employ a  point-in-polygon test, necessitating compiled C code for speed. For this purpose, I leverage the point-in-polygon command from the `sp` package:
...
Currently I have some helper functions to load, organize, clean and make some basic visualizations.
These are all made in base R and contained in the folder `./r/`. As the project develops and when there is more need to call C/C++ directly and classes can be more well-defined I will probably make this into a standalone package.
...
We begin by loading a `.log` file. These files are synchronized output between the Raspberry Pi OpenCV video recording and the Arduino that controls the conditioning protocol:
...
- `log$time` integer, is the hardware clock on the Arduino in milliseconds since the Arduino was rebooted. It can give you an estimate for how long the cells have been in the well before the start of the experiment. 
- `log$stimulus` character, name of the stimulus (e.g. `CS`, `US`, and protocol `start`)
...
The centroid is extracted by segmentation with a [U-net/ResNet (StarDist)](https://github.com/furthlab/paramecium_behavior/tree/main/models/grayscale_paramecium). OpenCV is then used to extract the centroid coordinate (x,y) of each label for each frame.
...
- `cent$ID` integer, unique label id for each frame. Note that ID does not track the cell identity over time it is based on the pixel coordinate while segmenting the label on each frame.    
...
Here `clean.bb()` is a command that removes bounding boxes where the cells have been lost. It does so by checking for the presence of a segmented centroid inside the bounding box and then flagging a variable `remove` with `TRUE`.
We can then subset this data into dat3.
...
Lets plot the trajectories for the second to the fourth second (fps = 30), so frame 30 to 90:
```{r}
...
Lastly lets superimpose the `.log` file onto the calculated velocity data.
...
The largest files are the `_shapes.csv` which contains the pixel outlines of each segmented cell (hence the morphology). The contour has been obtained through OpenCV via [cv::findContours](https://docs.opencv.org/3.4/df/d0d/tutorial_find_contours.html). 
...
Here we have a data set with `r format(nrow(shape), scientific = FALSE, big.mark = ',')` rows and `r format(length(unique(paste(shape$frame, shape$ID))), scientific = FALSE, big.mark = ',')` different contours.

Lets plot the shapes from frame 90 and superimpose them onto the tracks obtained from the DeepSort `15000mVSample03.csv` file.
...
Now that we have the basics down we can load two datasets and compare them visually.
...
```{r}
#| echo: false
log <- read.table('processed/15000mVSample03_timestamps.csv', header=TRUE, sep=',')
logTRC <- read.table('processed/TrulyRandomControlSample03_timestamps.csv', header=TRUE, sep=',')
  
vel <- read.table('processed/15000mVSample03_behavior.csv', header=TRUE, sep=',')
velTRC <- read.table('processed/TrulyRandomControlSample03_behavior.csv', header=TRUE, sep=',')
```
